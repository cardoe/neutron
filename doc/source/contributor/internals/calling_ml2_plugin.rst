...
This is a **FILO (first-in, last-out)** structure. The driver that binds level 0
is the first driver in the configured ``mechanism_drivers`` list that succeeds,
and subsequent levels are populated by drivers called in the same forward order.
...
``delete_port_postcommit`` notify all mechanism drivers in **reverse** configured
order — opposite to the forward order used by all other lifecycle callbacks
(``create_*``, ``update_*``).
...
level(s) they are responsible for. Drivers that did not participate in binding a
particular port will still be called (the full driver list is always walked); a
driver that has nothing to clean up for a given port should simply return.
